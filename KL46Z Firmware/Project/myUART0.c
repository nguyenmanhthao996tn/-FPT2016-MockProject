...
int UART0_init(uint32_t BaudRate, uint8_t DataBit, uint8_t ParityBit, uint8_t StopBit)
{
	...
	SIM->SOPT2 = (SIM->SOPT2 & ~SIM_SOPT2_UART0SRC_MASK) | SIM_SOPT2_UART0SRC(0x02U);					// UART0 clock = OSCERCLK clock
	...
	OSR_Cval = 4;
	SBR_Cval = (uint32_t)(8000000u / (OSR_Cval * BaudRate));
	
	if (BaudRate > (8000000u / (OSR_Cval * SBR_Cval)))
	{
		BaudDiff_Min = BaudRate - (8000000u / (OSR_Cval * SBR_Cval));
	}
	else
	{
		BaudDiff_Min = (8000000u / (OSR_Cval * SBR_Cval)) - BaudRate;
	}
	
	for ( i = 5; i < 32; i++)
	{
		OSR_val = i;
		SBR_val = (uint32_t)(8000000u / (OSR_val * BaudRate));
		
		if (BaudRate > (8000000u / (OSR_val * SBR_val)))
		{
			BaudDiff = BaudRate - (8000000u / (OSR_val * SBR_val));
		}
		else
		{
			BaudDiff = (8000000u / (OSR_val * SBR_val)) - BaudRate;
		}
	...
	// If the OSR is between 4x and 7x then both
  // edge sampling MUST be turned on.  
  //if ((OSR_Cval >3) && (OSR_Cval < 6))
	...
	UART0->C2 |= 	UART0_C2_RE_MASK | UART0_C2_RIE_MASK |
								UART0_C2_TE_MASK;
...
void UART0_printf1(const char *str, uint8_t size)
{
	int8_t i = size-1;
	for (; i >= 0; i--)
		UART0_Transmit(str[i]);
