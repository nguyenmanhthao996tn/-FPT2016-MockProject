...
void PIT_channel0_change_ldval(uint32_t ldval)  								 /* Set a new reload value of channel0 */
{
	PIT->CHANNEL[0].TCTRL &= ~PIT_TCTRL_TEN_MASK; /* Disable PIT */
	PIT->CHANNEL[0].LDVAL = ldval;                /* Load new ldvalue */
	PIT->CHANNEL[0].TCTRL |=  PIT_TCTRL_TEN_MASK; /* Enable PIT */
}

void PIT_channel1_change_ldval(uint32_t ldval)                   /* Set a new reload value of channel1 */
{
	PIT->CHANNEL[1].TCTRL &= ~PIT_TCTRL_TEN_MASK; /* Disable PIT */
	PIT->CHANNEL[1].LDVAL = ldval;                /* Load new ldvalue */
	PIT->CHANNEL[1].TCTRL |=  PIT_TCTRL_TEN_MASK; /* Enable PIT */
}
